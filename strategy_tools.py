...
def calculate_market_size(population: int, penetration_rate: float, avg_price: float) -> float: 
 ...
 Args: 
 population: The total number of potential users/customers in the market.  penetration_rate: The expected percentage of the population that will use the product (as a decimal, e.g., 0.05). 
...
def calculate_roic(net_income: float, invested_capital: float) -> float:  """ 
 Calculates the Return on Invested Capital (ROIC) as a percentage.  This is a key performance indicator for assessing capital efficiency. 
 Args: 
